...
public class Plip extends Creature {
    ...
    public Action chooseAction(Map<Direction, Occupant> neighbors) {
        ...
        for(Direction j:neighbors.keySet()){
            if(neighbors.get(j).name().equals("clorus")){
                Random random = new Random();
                int num = random.nextInt(2);
                if(num==0){
                    num=random.nextInt(emptyNeighbors.size());
                    List<Direction> x=new ArrayList<>();
                    for(Direction q:emptyNeighbors){
                        x.add(q);
                    }
                    return new Action(Action.ActionType.MOVE,x.get(num));
                }
            }
